...
from pipeline.utils.image_reade_inf import *
from pipeline.utils.ops import  *
from pipeline.utils.utils import *
from pipeline.utils.model_pgn import *

def get_human_parse(ID, input_dir: str='datasets/test/image', output_dir: str='datasets/test'):
    argp = argparse.ArgumentParser(description="Inference pipeline")
    argp.add_argument('-i',
                    '--directory',
                    type=str, help='Path of the input dir',
                    default=input_dir) # input 경로
    argp.add_argument('-o',
                    '--output',
                    type=str, help='Path of the input dir',
                    default=output_dir) # output 경로
    ...
    image_list_inp = []
    #for i in glob(os.path.join(data_path, '**'), recursive=True):
    for i in [os.path.join(data_path, f'{ID}.jpg')]:
        if os.path.isfile(i):
    ...
    RESTORE_FROM = './pipeline/checkpoint/CIHP_pgn' # 모델 경로
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 0.4  # 원하는 메모리 사용량 비율로 설정 (예: 0.5는 50%)
    ...
    # evaluate prosessing
    parsing_dir = os.path.join(output_path, 'image-parse-v3') # 최종 폴더
